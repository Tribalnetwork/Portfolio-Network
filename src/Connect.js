...
export default class Connect extends React.Component {
  ...
  requestingName = localStorage.getItem(
    "CognitoIdentityServiceProvider.1t8oqsg1kvuja9u9rvd2r1a6o4.LastAuthUser"
  );
  requestingUserData = localStorage.getItem(
    `CognitoIdentityServiceProvider.1t8oqsg1kvuja9u9rvd2r1a6o4.${this.requestingName}.userData`
  );
  ...
  createConnect = () => {
    ...
    API.graphql(
      graphqlOperation(mutations.createConnect, { input: data })
    ).then(() => {
      this.checkStatus();
    });
  };
  ...
  updateConnect = (requestingUser) => {
    API.graphql(
      graphqlOperation(queries.listConnects, {
        filter: {
          userId: {
      ...
      .then((result) => {
        ...
        this.setState({ bool: false });
        API.graphql(
          graphqlOperation(mutations.updateConnect, { input: data })
        ).then(() => {
          this.checkStatus();
        });
      });
  ...
  requestConnect = () => {
    if (this.state.bool == true) {
      ...
      API.graphql(
        ...
        if (
          result.data.listConnects == null ||
          result.data.listConnects.items.length == 0
        ) {
          this.createConnect();
        } else {
