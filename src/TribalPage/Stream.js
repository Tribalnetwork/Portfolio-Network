    ...
    try {
      ...
      request.onreadystatechange = async function () {
        if (this.readyState === 4 && this.status === 200) {
          ...
          const liveStream = await API.graphql(
            graphqlOperation(createLiveStream, { input: streamData })
          );
          const userData = {
      ...
      request.open(
        "POST",
        "https://7svcjtc33h.execute-api.us-east-1.amazonaws.com/test"
      );
      request.send();
    } catch (err) {
      console.log(err);
    ...
    <div>
      ...
      <div>
        {context.hasChannel ? (
          ...
          <button style={styles.button} onClick={generateUrl}>
            Create Channel
          </button>
        )}
      </div>
