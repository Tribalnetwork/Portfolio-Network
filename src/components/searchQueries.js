    ...
    constructor(){
       ...
       getUserNames = () => {
        ...
        search = (input, global) => {
           ...
           switch(item.type){
               case "User":
                    return <Link style={{textDecoration: "none"}} to={`/viewProfile?name=${item.name}&location=${item.location}&id=${item.id}`}><li key={item.id} style={this.liStyle}>{item.name}  <p>{item.type}</p></li></Link>
                    break;
               case "Film":
