    ...
    constructor(){
       ...
       getUserNames = () => {
        ...
        search = (input, global) => {
           ...
           switch(item.type){
                case "Film":
                    return <li key={item.id} style={this.filmLiStyle} onClick={RecentSearches.add('main', item)}><FilmFrame film={item} /></li>
                    break;
                case "Live Stream":
                   return <Link style={{textDecoration: "none"}} to={`/live?id=${item.id}`}><li key={item.id} style={this.liStyle} onClick={RecentSearches.add('main', item)}>{item.name}  <p style={this.typeStyle}>{item.type}</p></li> </Link>
                   break;
                case "User":
                    return <Link style={{textDecoration: "none"}} to={`/viewProfile?name=${item.name}&location=${item.location}&id=${item.id}`}><li key={item.id} style={this.liStyle} onClick={RecentSearches.add('main', item)}>{item.name}  <p style={this.typeStyle}>{item.type}</p></li></Link>
                    break;
           }
