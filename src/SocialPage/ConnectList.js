...
export default class ConnectList extends React.Component {
  ...
  getList = () => {
    axios
      .get("https://2h9u6s7401.execute-api.us-east-1.amazonaws.com/dev")
      .then((result) => {
  ...
  getUserNameByID = async (item) => {
    const user = await API.graphql(
      graphqlOperation(queries.getUser, { id: item.M.userId.S })
    );
  ...
  getList2 = (name, item) => {
    let element = [
      <li style={styles.li}>
        ...
        <p style={styles.status}>{item.M.status.S}</p>
      </li>,
    ];
